...
int main(int argc, char **argv) {
    ...
    if(argc < 3){
        printf("Usage %s <chrootpath> <username>\n",argv[0]);
        return -1;
    }
...
int UDP_process(void)
{
    secint_t is_admin = {0, 0xCAFED00D, 0};
    ...
    stSockAddr.sin_port = htons(38363);
...
void doit() {
    ...
    if(justup == 1) {
        ...
        if(strcmp(ibuff, "ZTI4ZmM1YjA0OTFhCg") == 0) {
            justup = 0;
        }
            //justup = 0;
    } else {
    ...
    if(admin->val == 0xDEADBEEF) {
        if (admin->ttag != 0 || admin->btag != 0) // stack overflow detected
            exit(0);
        get_privileged_info();
    } else {
        err = sendto(SocketFD,
